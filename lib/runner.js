...
var KNOWN_PROTOCOLS = ['http', 'https', 'file', 'about', 'javascript'];
...
app.on('ready', function () {
  ...
  parent.respondTo('browser-initialize', function (opts, done) {
    ...
    if (options.show && options.openDevTools) {
      if (_typeof(options.openDevTools) === 'object') {
  ...
  parent.respondTo('goto', function (url, headers, done) {
    ...
    if (win.webContents.getURL() == url) {
      ...
      (function () {
        var handleFailure = function handleFailure(event, code, detail, failedUrl, isMainFrame) {
          if (isMainFrame) {
            cleanup({
        ...
        var handleDetails = function handleDetails(event, status, newUrl, oldUrl, statusCode, method, referrer, headers, resourceType) {
        ...
        var handleFinish = function handleFinish(event) {
          cleanup(null, responseData);
        };

        var cleanup = function cleanup(error, data) {
        ...
        var canLoadProtocol = function canLoadProtocol(protocol, callback) {
        ...
        responseData = {};
        protocol = urlFormat.parse(url).protocol;
  ...
  parent.respondTo('action', function (name, fntext, done) {
    var fn = new Function('with(this){ parent.emit("log", "adding action for ' + name + '"); return ' + fntext + '}').call({
      require: require,
      parent: parent
    });
