...
function Nightmare(options) {
  ...
  this.proc.stdout.pipe(split2()).on('data', function (data) {
    electronLog.stdout(data);
  });

  this.proc.stderr.pipe(split2()).on('data', function (data) {
...
proto.enterIframe = function (selector) {
  this.wait(selector).enterIframeProxy(selector)
  return this
}
