...
var RENDER_ELEMENT_ID = '__NIGHTMARE_RENDER__';
...
function FrameManager(window) {
  ...
  this.requestFrame = function (callback) {
    ...
    if (!requestedFrame) {
      ...
      requestedFrame = true;
      window.webContents.executeJavaScript('(' + triggerRender + ')("' + RENDER_ELEMENT_ID + '")');
    }
  };
};
...
var triggerRender = function (id) {
  ...
  if (renderElement) {
    ...
    renderElement.id = id;
    renderElement.setAttribute('style', 'position: absolute;' + 'left: 0;' + 'top: 0;' + 'width: 1px;' + 'height: 1px;');
